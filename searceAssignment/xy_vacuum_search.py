...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        #
        # return curNode.path_cost = turn left or right + go next cell
        step_cost = 1  # Cost of moving to the next cell
        if self.turnCostOn:
            turn_cost = self.computeTurnCost(curNode.action, action)
        else:
            turn_cost = 0
        return curNode.path_cost + step_cost + turn_cost

    def computeTurnCost(self, action1, action2):
        # no movement or turn on
        if action1 == action2:
            return 0
        if {action1, action2} in [{'UP', 'DOWN'}, {'LEFT', 'RIGHT'}]:
            return 100 # 180-degree turn
        return 50  # 90-degree turn
    ...
    def findMinManhattanDist(self, pos):
        ...
        #
        #
        """Finds the minimum Manhattan distance to the closest dirty room."""
